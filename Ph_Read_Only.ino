...
const int analogInPin = A0;     //Analog pH signal comes from A1. Make sure you plug it correctly, or change to another number if you really must
unsigned long int sumvalue;     //Sum value of 10 analog measures taken. Unsigned means it is always positive. Long means it is 32 bit rather than 16 bit.
 ...
 //Initial message requesting calibration (Change to a more polite message when sharing the code. Or don't. I like it this way.)
  ...
  Serial.print("I need to calibrate myself!");
   ...
   lcd.print("Reading");
	...
	for(int i=0;i<9;i++) //Sorting the array. There is probably some .sort method that could do this automatically.
 		{
